...
void check_nat(struct nf_conntrack *ct, struct conn_key *c_key, struct conn_value *c_value) {
    ...
    if (nfct_getobjopt(ct, NFCT_GOPT_IS_SNAT)) {
        ...
        // Check if reverse
        if (c_key->src_ip == nfct_get_attr_u32(ct, ATTR_ORIG_IPV4_SRC)) {
            old_ip = nfct_get_attr_u32(ct, ATTR_ORIG_IPV4_SRC);
            new_ip = nfct_get_attr_u32(ct, ATTR_REPL_IPV4_DST);

            c_value->n_entry.src_ip        = new_ip;
            c_value->n_entry.rewrite_flag |= REWRITE_SRC_IP;
        }
        else {
            // Reverse (DNAT)
            ...
            c_value->n_entry.dest_ip       = new_ip;
            c_value->n_entry.rewrite_flag |= REWRITE_DEST_IP;
        }

        // Calculate the L3 and L4 checksum diffs
        csum_replace4(&c_value->l3_cksum_diff, old_ip, new_ip);
    ...
    if (nfct_getobjopt(ct, NFCT_GOPT_IS_DNAT)) {
        ...
        // Check if reverse
        if (c_key->dest_ip == nfct_get_attr_u32(ct, ATTR_ORIG_IPV4_DST)) {
    ...
    if (nfct_getobjopt(ct, NFCT_GOPT_IS_SPAT)) {
        ...
        // Check if reverse
        if (c_key->src_port == nfct_get_attr_u16(ct, ATTR_ORIG_PORT_SRC)) {
            old_port = nfct_get_attr_u16(ct, ATTR_ORIG_PORT_SRC);
            new_port = nfct_get_attr_u16(ct, ATTR_REPL_PORT_DST);

            c_value->n_entry.src_port      = new_port;
            c_value->n_entry.rewrite_flag |= REWRITE_SRC_PORT;
        }
        else {
            // Reverse (DPAT)
            ...
            c_value->n_entry.dest_port     = new_port;
            c_value->n_entry.rewrite_flag |= REWRITE_DEST_PORT;
        }

        // Calculate the L4 checksum diff
        csum_replace2(&c_value->n_entry.l4_cksum_diff, old_port, new_port);
    }
    ...
    if (nfct_getobjopt(ct, NFCT_GOPT_IS_DPAT)) {
        ...
        // Check if reverse
        if (c_key->dest_port == nfct_get_attr_u16(ct, ATTR_ORIG_PORT_DST)) {
